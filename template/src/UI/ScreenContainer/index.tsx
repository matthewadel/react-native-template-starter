...
import { KeyboardAvoidingView, Platform, ViewStyle, ScrollView } from 'react-native'
...
const ScreenContainer = (props: IScreenContainer) => {
  return (
    <SafeAreaView edges={['top']} style={[{ height: '100%' }, props.containerStyle]}>

      <ScreenHeader {...props.headerProps} />

      <KeyboardAvoidingView style={{ width: '100%', flex: 1 }} behavior={Platform.OS == 'android' ? 'height' : undefined}>
        <ScrollView scrollEnabled={true} nestedScrollEnabled={true} style={{ flexGrow: 1, }} contentContainerStyle={[{ width: '100%', height: "100%", alignItems: 'center', alignSelf: 'center' }, props.noDirectionChange ? ConvertStyleToObject(props.style) : ChangeDirectionStyle((props.style))]}
